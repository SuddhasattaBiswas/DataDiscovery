...
if spark.catalog.tableExists("ndh.ROU_ASSET_NBV_NDT"):
    max_timestamp_df = spark.table("ndh.ROU_ASSET_NBV_NDT").select(max("CREATE_DATE").alias("max_timestamp"))
...
mergedf = spark.read.option("mergeSchema", "true").parquet("/mnt/ADLS1/PREP/1stParty/BUSINESS_FEED_POWERPLAN/Sensitive/ROU_NBV/*/*/*/*.parquet")
dict_trans = [
        ('OM',{'path_name':'/mnt/ADLS1/PREP/1stParty/BUSINESS_FEED_POWERPLAN_OM/Sensitive/ROU_NBV/'}),
        ('PH',{'path_name':'/mnt/ADLS1/PREP/1stParty/BUSINESS_FEED_POWERPLAN_PH/Sensitive/ROU_NBV/'}),
        ('PK',{'path_name':'/mnt/ADLS1/PREP/1stParty/BUSINESS_FEED_POWERPLAN_PK/Sensitive/ROU_NBV/'})
        ]
...
master_df=master_df.withColumnRenamed('ROU_Asset_NBV','ROU_ASSET_NET_BOOK_VALUE')
...
table = 'dbo.ROU_ASSET_NBV_NDT'
appendToSynapse(result_df,table)

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from ndh.ROU_ASSET_NBV_NDT

# COMMAND ----------

result_df.createOrReplaceTempView('result_table')
df_write_incr = spark.sql("""Insert into ndh.ROU_ASSET_NBV_NDT (select * from result_table)""")

# COMMAND ----------

AuditTableUpdate('ROU_ASSET_NBV')
