...
print("Initial distinct count : ",LEASE_DATA_df.count())

LEASE_DATA_df = spark.sql(QUERY_CST_PROPERTY_ID_TX)

print("After getting Property ID : ",LEASE_DATA_df.count())
...
print("After getting country codes : ",LEASE_DATA_df.count())

LEASE_DATA_df = LEASE_DATA_df.join(PEOPLE_df,LEASE_DATA_df.CST_PEOPLE_TX == PEOPLE_df.TRI_USER_NAME_TX,"left")

print("After getting user names : ",LEASE_DATA_df.count())
...
#LEASE_DATA_NDH_df = spark.sql(QUERY_LEASE_DATA)

print("Initial count : ",SBR_QUANTITY_RULES_df.count())

SBR_QUANTITY_RULES_df = SBR_QUANTITY_RULES_df.join(LEASE_DATA_df,SBR_QUANTITY_RULES_df.TRI_LEASE_ID_TX == LEASE_DATA_df.Lease_Id,"left")

print("After getting the country code : ",SBR_QUANTITY_RULES_df.count())
...
SBR_QUANTITY_RULES_df = SBR_QUANTITY_RULES_df.select(
	...
	,col("TRI_LEASE_ID_TX").alias("Lease_Id")
...
# MAGIC %sql
# MAGIC
# MAGIC SHOW COLUMNS FROM NDH.SBR_VALUE_RULES_NDT
