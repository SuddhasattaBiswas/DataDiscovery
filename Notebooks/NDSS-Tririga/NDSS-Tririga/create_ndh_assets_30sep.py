...
# LEASE_CLAUSE_df = spark.sql('select * from(SELECT *  ,ROW_NUMBER()  OVER(PARTITION BY TRI_ID_TX,TRI_CONTROL_NUMBER_CN ORDER BY TRI_MODIFIED_DATE_SY DESC) AS ROW_NUM FROM LEASE_CLAUSE_vw sort by ROW_NUM)where ROW_NUM = 1')
...
#LEASE_DATA_df.filter(TRI_STATUS_CL == 'Active')
...
# MAGIC %sql select * from final where Lease_Clause_Legacy_Id = 1005165

# COMMAND ----------

# %sql select distinct Lease_Clause_Legacy_Id,tri_status from final where upper(tri_status) = 'TERMINATED' 

# COMMAND ----------

# %sql select distinct tri_status,Lease_Clause_Legacy_Id from final 
# --retired- 1012896, active - 1005080, Terminated-1004990
...
# %sql select distinct(status) from value_data
