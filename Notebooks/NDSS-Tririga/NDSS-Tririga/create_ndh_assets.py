...
# DBTITLE 1,To get latest data for lease_clause
LEASE_CLAUSE_df.createOrReplaceTempView("lease_clause_vw")
# LEASE_CLAUSE_df = spark.sql('select * from(SELECT *  ,ROW_NUMBER()  OVER(PARTITION BY TRI_LEGACY_CLAUSE_ID_TX ORDER BY TRI_MODIFIED_DATE_SY DESC) AS ROW_NUM FROM LEASE_CLAUSE_vw sort by ROW_NUM)where ROW_NUM = 1')
LEASE_CLAUSE_df = spark.sql('select * from(SELECT p.*  ,ROW_NUMBER()  OVER(PARTITION BY TRI_ID_TX,TRI_CONTROL_NUMBER_CN ORDER BY TRI_MODIFIED_DATE_SY DESC) AS ROW_NUM FROM LEASE_CLAUSE_vw as p)as t where ROW_NUM = 1')
LEASE_CLAUSE_df.createOrReplaceTempView("lease_clause_status")
LEASE_CLAUSE_df = spark.sql("select * from lease_clause_status where lower(TRI_STATUS_CL) = 'active'")



# COMMAND ----------

# DBTITLE 1,To get latest records for lease_data
LEASE_DATA_df.createOrReplaceTempView("lease_data_vw")
LEASE_DATA_df = spark.sql('select * from(SELECT p.*  ,ROW_NUMBER()  OVER(PARTITION BY TRI_ID_TX,TRI_CONTROL_NUMBER_CN ORDER BY TRI_MODIFIED_DATE_SY DESC) AS ROW_NUM FROM lease_data_vw as p)as t where ROW_NUM = 1')
LEASE_DATA_df.createOrReplaceTempView("lease_data_status")
# LEASE_DATA_df = spark.sql("select * from lease_data_status where lower(TRI_STATUS_CL) = 'active'")



# COMMAND ----------

# DBTITLE 1,To get the latest records for sbr_qty_rules

SBR_QUANTITY_RULES_df.createOrReplaceTempView("sbr_qty_vw")
SBR_QUANTITY_RULES_df = spark.sql('select * from(SELECT p.*  ,ROW_NUMBER()  OVER(PARTITION BY TRI_ID_TX ORDER BY TRI_MODIFIED_DATE_SY DESC) AS ROW_NUM FROM sbr_qty_vw as p)as t where ROW_NUM = 1')
...
LEASE_DATA_df = LEASE_DATA_df.select(
  ...
  ,col("TRI_EXPIRATION_DA").cast("timestamp").alias("Lease_Expiration_Date")
  ,col("TRI_ORIGINAL_EXPIRATION_DA").cast("timestamp").alias("Original_Contract_Expiration_Date")
  ,col("TRI_ORIGINAL_START_DA").cast("timestamp").alias("Original_Contract_Start_Date")
  ,col("TRI_RENT_COMMENCE_DA").cast("timestamp").alias("Rent_Commencement_Date")
  ,col("TRI_START_DA").cast("timestamp").alias("Contract_Start_Date")
  ...
  ,col("TRI_MODIFIED_DATE_SY").cast("timestamp").alias("Source_Last_Modified_Date")
...
SBR_QUANTITY_RULES_df = SBR_QUANTITY_RULES_df.filter((SBR_QUANTITY_RULES_df.TRI_STATUS_CL == 'Active'))

print("After status filter : ",SBR_QUANTITY_RULES_df.count())

SBR_QUANTITY_RULES_df = SBR_QUANTITY_RULES_df.select(
#   col("TRI_STATUS_CL").alias("tri_status")
...
SBR_QUANTITY_RULES_df.createOrReplaceTempView("final")

# COMMAND ----------

# MAGIC %sql select * from final 
...
SBR_VALUE_RULES_df = SBR_VALUE_RULES_df.filter((SBR_VALUE_RULES_df.TRI_STATUS_CL == 'Active') 
#                                                | (SBR_VALUE_RULES_df.TRI_STATUS_CL == 'Retired')
                                              )
...
SBR_VALUE_RULES_df = SBR_VALUE_RULES_df.select(
	 col("TRI_CLAUSE_ID_TX").alias("Lease_Clause_Legacy_Id")
    #,col("TRI_STATUS_CL").alias("status")
	,col("TRI_SALES_CATEGORY_CL").alias("Sales_Category")
...
SBR_VALUE_RULES_df.write.format('delta').option("overwriteSchema", "true").mode('overwrite').save(SBR_VALUE_RULES_cur)

# COMMAND ----------

df = spark.read.format("delta").load("/mnt/ADLS2/NDH/NonSensitive/TRIRIGA/SBR_VALUE_RULES_NDT")
df.createOrReplaceTempView("SBR_VALUE_RULES_NDT_temp")
display(df)

# COMMAND ----------

# MAGIC %sql select * from NDH.LEASE_DATA_NDT
# MAGIC

# COMMAND ----------

# MAGIC %sql select * from NDH.SBR_QUANTITY_RULES_NDT

# COMMAND ----------

# MAGIC %sql select * from NDH.SBR_VALUE_RULES_NDT
